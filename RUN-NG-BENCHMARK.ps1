...
# --- Find script directory (compatible with PowerShell 5.1+) ---
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Definition
if (-not $ScriptDir) { $ScriptDir = (Get-Location).Path }
$ProjectDir = $ScriptDir

# --- Find binary in bin\ folder ---
$simBinary = Join-Path $ScriptDir 'bin\sim_stress_v43ext.exe'

if (-not (Test-Path $simBinary)) {
    Write-Host ''
    Write-Host '================================================================' -ForegroundColor Red
    Write-Host '  CRITICAL ERROR: Binary not found!' -ForegroundColor Red
...
Write-Host '  NeuroGraph ANGP v4.3.1-FIXED - BINARY READY' -ForegroundColor Cyan
Write-Host '=============================================================' -ForegroundColor Cyan
Write-Host ''
Write-Host "  Binary: $simBinary" -ForegroundColor Green
Write-Host "  CPU cores: $coreCount" -ForegroundColor DarkGray
Write-Host ''
